<?php 
		...
		if (strpos($user3['permissions'], "COMMAND") == false) {
	...
	if(isset($_GET['do']) && $_GET['do'] == "add") {
		...
		$words = $user2['blockcommands'];
		
		$newable = $_POST['word'] . "," . $words;
		$statement = $pdo->prepare("UPDATE ACP SET blockcommands= :block WHERE vsdnc= 1");
	...
	function echoWords() {
		...
		if($user2 == null || $user2['blockcommands'] == "") {
			echo "<center><div class='card grey darken-2 white-text'><div class='card-content white-text'><i>Keine W&oumlrter eingetragen!</i></div></div></center>";
		}else {
		$words = $user2['blockcommands'];
		...
		foreach($splitted as $s) {
			...
			<div class="right">
				<a href="?go=blockedcmds&delete=' . $s . '"class="btn-floating btn-large red"><i class="material-icons">remove</i></a>
				</div>
			</div></div> <br \> <br \>';
		}
		}
...
<span class="card-title"><center>Alle blacklisted Commands</center></span>
<br \>
<form action="?go=blockedcmds&do=add&update=true" method="post">
